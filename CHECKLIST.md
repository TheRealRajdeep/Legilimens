...
- [x] **S2** Deployed to Subgraph Studio as `guessworker` v0.0.1. Query URL: `https://api.studio.thegraph.com/query/1760267/guessworker/v0.0.1`.
  - Synced with no indexing errors, and all 3 testnet games are indexed.
  - `SUBGRAPH_URL` and `NEXT_PUBLIC_SUBGRAPH_URL` are set in `web/.env.local`.
  - To redeploy: `cd subgraph && npx graph deploy guessworker --version-label v0.0.X`. *(me)*
